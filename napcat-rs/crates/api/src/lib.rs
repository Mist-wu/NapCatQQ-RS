...
mod tests {
    ...
    #[tokio::test]
    async fn api_get_events_route_filters_trimmed_type_list() {
        let state = ApiState::new();
        let app = state.clone().router();
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/get_events?type=%20message,%20meta_event&timeout_ms=50&max_events=8")
                    .body(Body::empty())
                    .expect("valid request"),
            )
            .await
    ...
    #[tokio::test]
    async fn api_get_events_route_type_takes_precedence_over_post_type() {
        let state = ApiState::new();
        let app = state.clone().router();
    ...
    #[tokio::test]
    async fn api_get_events_route_does_not_fallback_to_post_type_when_type_is_invalid() {
        let state = ApiState::new();
        let app = state.clone().router();
    ...
    #[tokio::test]
    async fn api_get_group_info_rejects_whitespace_group_id() {
        let state = ApiState::new();
        let app = state.clone().router();
        let payload = serde_json::to_string(&GetGroupInfoRequest {
        ...
        assert_eq!(envelope["status"], "failed");
        assert_eq!(envelope["retcode"], -1);
        assert_eq!(envelope["message"].as_str(), Some("group_id is required"));
    }
